...
export type consts = { kind: "const", value: Value }
...
export function is_number(t: Type) {
    if (t.kind === "consts") return is_number(t.value.type)
...
export function format_type(t: Type): string {
    ...
    if (t.kind === "struct") return `struct<${Object.entries(t.fields).map(([name, type]): string => `${name}:${format_type(type)}`).join("::")}>`
    return t.kind
}

export function type_to_wasm(t: Type) {
    if (t.kind === "consts") return type_to_wasm(t.value.type)
...
export function assert_type_eq(a: Type, b: Type, message: string="Types not equal!") {
    if (a.kind === "consts") return assert_type_eq(a.value.type, b, message)
    if (b.kind === "consts") return assert_type_eq(a, b.value.type, message)
